 ...
 */
export function registerExtendedTools(server, client) {
    // ----- Core -----
    ...
    server.registerTool("get_user_flipwallets", {
        title: "Get User Flipwallets",
        description: "Get the Flipwallets (per-chain wallet addresses) of a given Discord user. Flipwallets can change over time, so don't cache them.",
    ...
    server.registerTool("get_user", {
        ...
        description: "Get details about a user who has verified in your server: Discord profile, and the linked accounts and wallets required by your gating rules. Users can opt out of sharing (fields come back redacted). Requires the Growth plan.",
    ...
    // ----- Points -----
    server.registerTool("list_point_systems", {
        title: "List Point Systems",
        description: "List the point systems in your community, with optional name search, status/permission filters, and pagination. Use this to find a point system's ID.",
    ...
    server.registerTool("get_points_balance", {
        title: "Get Points Balance",
        description: "Get the balance of a point system for one or more users (or the top holders). Filter by specific user IDs and sort by balance; paginate with offset/limit (max 100).",
    ...
    server.registerTool("update_points_balance", {
        title: "Update Points Balance",
        description: "Credit or debit a point system balance for one or more users. Each change has a Discord userId and an amount (positive to add, negative to subtract). This permanently modifies user balances in your community.",
    ...
    // ----- Treasury reads -----
    server.registerTool("get_tipping_wallet_balance", {
        title: "Get Tipping Wallet Balance",
        description: "Get your community's shared tipping wallet token balances on a given chain, including symbol, normalized and raw balance, and USD value.",
    ...
    server.registerTool("get_tipping_wallet_inventory", {
    ...
    server.registerTool("send_tip", {
        title: "Send Tip",
        description: "Send a tip (points, token, or NFT) from your shared tipping wallet to a Discord user, posted in a channel. WARNING: token and NFT tips move real on-chain assets and cannot be undone. Confirm the recipient, item, and amount before calling.",
    ...
    server.registerTool("transfer_asset", {
        title: "Transfer Asset",
        description: "Transfer a token or NFT from your shared tipping wallet to an arbitrary wallet address. WARNING: this moves real on-chain assets to an external address and cannot be undone. Double-check the recipient address, item, and amount before calling.",
    ...
    server.registerTool("create_airdrop", {
        title: "Create Airdrop",
        description: "Create a timed airdrop of points or a token in a Discord channel; entrants split or claim the prize when the countdown ends. WARNING: token airdrops commit real on-chain assets. Confirm the item, amount, countdown, and entry limits before calling.",
    ...
    server.registerTool("create_raffle", {
        title: "Create Raffle",
        description: "Create a timed raffle of points, a token, an NFT, or a Discord role in a channel; random winners are picked when the countdown ends. WARNING: token/NFT raffles commit real on-chain assets. Confirm the item, winners, countdown, and entry limits before calling.",
    ...
    server.registerTool("burn_asset", {
        title: "Burn Asset",
        description: "Permanently burn a token or NFT held in your shared tipping wallet. WARNING: burning is irreversible and destroys the asset. Confirm the exact item and amount before calling.",
...
//# sourceMappingURL=extended-tools.js.map