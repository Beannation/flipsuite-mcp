...
export function registerQuestTools(server, client) {
    ...
    server.registerTool("get_quest", {
        title: "Get Quest",
        description: "Fetch full details of a single quest in your FlipSuite community by its quest ID, including its tasks, rewards, status, timing, and completion counts.",
    ...
    server.registerTool("list_quests", {
        title: "List Quests",
        description: "List quests in your FlipSuite community. Optionally filter by status, recurrence type, or visibility, and paginate with offset/limit. Use this to find a quest's ID before fetching its details.",
    ...
    server.registerTool("get_quest_completions", {
        title: "Get Quest Completions",
        description: "Retrieve quest completion records for your FlipSuite community, including who completed which quest, when, and the verification data per task. Filter by quest ID, quest round (iteration), user ID, and time range; paginate with offset/limit (max 50 per page).",
